...
def get_ffn2(model, index):
    ...
    ffn2 = layer.mlp.down_proj  # Llama usa down_proj ao invés de c_proj
    return ffn2
...
def get_attention_output(model, index):
    layer = get_layers(model)[index]
    output = layer.self_attn  # Llama usa self_attn ao invés de attn
    return output


def get_layers(model):
    decoder = get_backbone(model)
    layers = decoder.layers  # Llama usa layers ao invés de h
    return layers
